...
Includes interpretable_drift tests (InterpretableDriftDetector v0.7.0).
"""
...
WORKSPACE_PATH = "/Workspace/insurance-monitoring-v070"
PROJECT_ROOT = pathlib.Path("/home/ralph/insurance-monitoring")
...
# Upload the test runner notebook (v070 version with correct workspace path)
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_v070.py"
if nb_local.exists():
    upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_v070")
else:
    print(f"WARNING: notebook not found at {nb_local}")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v0.7.0",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_v070",
            ),
        )
    ],
    environments=[
