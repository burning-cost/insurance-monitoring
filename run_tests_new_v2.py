"""
Run new insurance-monitoring test files on Databricks serverless compute.

Tests: test_drift_v2, test_discrimination_v4, test_drift_attribution_v2,
       test_interpretable_drift_v2, test_calibration_plots
"""
...
# Upload the notebook
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_new_v2.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_new_v2")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-new-tests-v2",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_new_v2",
            ),
        )
    ],
    environments=[
