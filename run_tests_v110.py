"""
Run insurance-monitoring v0.11.0 test suite (conformal_chart) on Databricks serverless compute.
"""
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_v110.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_v110")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v0.11.0-conformal-chart",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_v110",
            ),
        )
    ],
    environments=[
