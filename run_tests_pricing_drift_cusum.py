"""
Run insurance-monitoring v0.10.0 test suite (PricingDriftMonitor and
CalibrationCUSUM) on Databricks serverless compute.
"""
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_pricing_drift_cusum.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_pricing_drift_cusum")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v0.10.0",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_pricing_drift_cusum",
            ),
        )
    ],
    environments=[
