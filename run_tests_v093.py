"""
Run insurance-monitoring v0.9.3 test suite (MulticalibrationMonitor) on Databricks.

Uploads all source and test files, then runs the full pytest suite via a notebook task.
"""
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_v093.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_v093")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v0.9.3-multicalibration",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_v093",
            ),
        )
    ],
    environments=[
