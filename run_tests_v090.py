"""
Run insurance-monitoring v0.9.0 test suite on Databricks serverless compute.
Includes MLflow tracker tests (all mocked — no real tracking server needed).
"""
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_v090.py"
if nb_local.exists():
    upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_v090")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v0.9.0",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_v090",
            ),
        )
    ],
    environments=[
