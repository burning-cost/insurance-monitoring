"""
Run insurance-monitoring v0.8.0 test suite on Databricks serverless compute.
Tests the sequential module (mSPRT) alongside the full existing test suite.
"""
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_v080.py"
if nb_local.exists():
    upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_v080")
else:
...
run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v0.8.0",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_v080",
            ),
        )
    ],
    environments=[
