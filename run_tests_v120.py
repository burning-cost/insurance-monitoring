"""
Run insurance-monitoring v0.12.0 test suite (conformal_spc) on Databricks serverless compute.

New in v0.12.0: ConformedControlChart, ConformedProcessMonitor,
ConformedControlResult, ConformedMonitorResult (conformal_spc.py).
"""
...
# Collect src/, tests/, and pyproject.toml
upload_files = []
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_v120.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_v120")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v0.12.0-conformal-spc",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_v120",
            ),
        )
    ],
    environments=[
