"""
Run coverage-gap tests for insurance-monitoring on Databricks serverless compute.

Uploads src/, tests/ (including the new test_coverage_gaps.py), and the notebook.
Submits a serverless job and polls until completion.
"""
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_coverage_gaps.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_coverage_gaps")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-coverage-gap-tests",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_coverage_gaps",
            ),
        )
    ],
    environments=[
