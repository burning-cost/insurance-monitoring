"""
Run insurance-monitoring coverage expansion test suite on Databricks serverless compute.
New tests: test_coverage_gaps_v2.py — April 2026.
"""
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_coverage_v2.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_coverage_v2")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-coverage-v2-april-2026",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_coverage_v2",
            ),
        )
    ],
    environments=[
