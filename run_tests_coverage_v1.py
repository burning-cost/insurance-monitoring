"""
Run insurance-monitoring test suite with coverage on Databricks serverless compute.
Designed to measure coverage improvement from new test files.
"""
...
# Also upload pyproject.toml and coverage notebook
...
# Upload coverage notebook
coverage_nb_path = PROJECT_ROOT / "notebooks" / "run_tests_coverage.py"
if coverage_nb_path.exists():
    upload_notebook(coverage_nb_path, f"{WORKSPACE_PATH}/notebooks/run_tests_coverage")

print("\nSubmitting coverage test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-coverage-v1",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_coverage",
            ),
        )
    ],
    environments=[
