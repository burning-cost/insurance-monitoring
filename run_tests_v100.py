"""
Run insurance-monitoring v1.0.0 test suite (GiniDriftTest) on Databricks serverless compute.
"""
...
# Collect src/, tests/test_gini_drift.py, and pyproject.toml
upload_files = []
...
# Only upload the new test file (plus keep full test suite for context)
...
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_v100.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_v100")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v1.0.0-gini-drift",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_v100",
            ),
        )
    ],
    environments=[
