"""
Run insurance-monitoring v1.1.0 BAWSMonitor test suite on Databricks serverless compute.
"""
...
# Upload the BAWS test notebook
nb_local = PROJECT_ROOT / "notebooks" / "run_tests_baws.py"
upload_notebook(nb_local, f"{WORKSPACE_PATH}/notebooks/run_tests_baws")

print("\nSubmitting test job (serverless compute)...")

run = w.jobs.submit(
    run_name="insurance-monitoring-tests-v1.1.0-baws",
    tasks=[
        jobs.SubmitTask(
            task_key="run_tests",
            environment_key="default",
            notebook_task=jobs.NotebookTask(
                notebook_path=f"{WORKSPACE_PATH}/notebooks/run_tests_baws",
            ),
        )
    ],
    environments=[
